...
export function DestinationsCard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      country: form.country.trim().toUpperCase().slice(0, 2) || "ES",
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Error al guardar destino");
    }
  };

  const handleDelete = async (d: Destination) => {
    if (!confirm(`¿Eliminar el destino "${d.name}"?`)) return;
    try {
      await del.mutateAsync(d.id);
      toast.success("Destino eliminado");
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Error al eliminar destino");
    }
  };
    ...
    <div className="rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-text-primary">Destinos</h3>
          <span className="text-xs text-text-secondary">{destinations.length}</span>
        </div>
        <button
          onClick={openCreate}
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="rounded-lg border border-border p-4 space-y-3 bg-surface/30">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div>
                <label className="block text-xs text-text-secondary mb-1">País (ISO 2)</label>
                <input
                  type="text"
                  maxLength={2}
                  value={form.country}
              ...
              <div className="sm:col-span-3">
                <label className="block text-xs text-text-secondary mb-1">Región (opcional)</label>
                <input
                  type="text"
                  value={form.region}
                  onChange={(e) => setForm({ ...form, region: e.target.value })}
                  className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-coral focus:outline-none"
                  placeholder="Cataluña, Andalucía…"
                />
              </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-surface/50">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-text-secondary">Región</th>
                  <th className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-text-secondary">País</th>
                ...
                {destinations.map((d) => (
                  <tr key={d.id} className="hover:bg-surface/30 transition-colors">
                    <td className="px-4 py-3 text-sm">
                      ...
                      <div className="text-xs text-text-secondary font-mono">{d.slug}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-text-secondary">{d.region ?? "—"}</td>
                    <td className="px-4 py-3 text-sm text-text-secondary">{d.country}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${d.isActive ? "bg-sage-light text-sage" : "bg-warm-muted text-text-secondary"}`}>
                        {d.isActive ? "Activo" : "Inactivo"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(d)} className="rounded-lg p-1.5 text-text-secondary hover:bg-warm-muted hover:text-coral transition-colors" title="Editar">
                          <Pencil className="h-4 w-4" />
                        </button>
                        <button onClick={() => handleDelete(d)} className="rounded-lg p-1.5 text-text-secondary hover:bg-red-50 hover:text-danger transition-colors" title="Eliminar">
